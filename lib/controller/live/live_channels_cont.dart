...
class LiveChannelsControllerImp extends LiveChannelsController {
  ...
  getLiveCatChild() async {
    try {
      ...
      if(user == null){
        Get.defaultDialog(title: "Error", middleText: "Server Error!! -> Live Channels");
        return;
      }

      var url = "${user.serverInfo!.serverUrl}/player_api.php?password=${user.userInfo!.password}&username=${user.userInfo!.username}&action=get_live_streams";
  ...
  getLiveCatChilds(CatId) async {
    try {
      ...
      var url = "${user.serverInfo!.serverUrl}/player_api.php?password=${user.userInfo!.password}&username=${user.userInfo!.username}&action=get_live_streams&category_id=$CatId";
      var liveCatChilds = await initData.getLiveCatChilds(url);
  ...
  applyFirstCat() async {
    ...
    var url = "${user.serverInfo!.serverUrl}/player_api.php?password=${user.userInfo!.password}&username=${user.userInfo!.username}&action=get_live_categories";
