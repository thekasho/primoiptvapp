...
class SeriesCatsControllerImp extends SeriesCatsController {
  ...
  getCats() async {
    try {
      ...
      if(user == null){
        Get.defaultDialog(title: "Error", middleText: "Server Error!! -> Series");
        return;
      }

      var url = "${user.serverInfo!.serverUrl}/player_api.php?password=${user.userInfo!.password}&username=${user.userInfo!.username}&action=get_series_categories";
