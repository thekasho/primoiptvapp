...
class SeriesChannelsControllerImp extends SeriesChannelsController {
  ...
  getSeries(catId) async {
    try {
      ...
      if(user == null){
        Get.defaultDialog(title: "Error", middleText: "Server Error!! -> Series Channels");
        return;
      }

      var url = "${user.serverInfo!.serverUrl}/player_api.php?password=${user.userInfo!.password}&username=${user.userInfo!.username}&action=get_series&category_id=$catId";
