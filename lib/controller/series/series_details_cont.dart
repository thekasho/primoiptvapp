...
class SeriesDetailsControllerImp extends SeriesDetailsController {
  ...
  addFav(seriesId) async {
    try {
      ...
      if(user == null){
        Get.defaultDialog(title: "Error", middleText: "Server Error!! -> Series Details");
        return;
      }

      var url = "${user.serverInfo!.serverUrl}/player_api.php?password=${user.userInfo!.password}&username=${user.userInfo!.username}&action=get_series_info&series_id=$seriesId";
      ...
      var caturl = "${user.serverInfo!.serverUrl}/player_api.php?password=${user.userInfo!.password}&username=${user.userInfo!.username}&action=get_series&category_id=$catId";
  ...
  getAuthLink() async {
    ...
    if(user != null){
      final link =
          "${user.serverInfo!.serverUrl}/series/${user.userInfo!.username}/${user.userInfo!.password}/";
      print(link);
      return link;
    } else {
